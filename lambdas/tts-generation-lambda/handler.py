...
def store_audio_chunk(job_id: str, chapter_num: int, chunk_index: int,
                     wav_data: bytes, chapter_title: str, total_chunks: int) -> str:
    """
    ...
    """
    audio_key = f"{job_id}/chapter_{chapter_num:02d}/audio_chunk_{chunk_index:03d}.wav"
...
def send_to_stitch_queue(job_id: str, chapter_num: int, chapter_title: str,
    ...
    dedup_id = f"{job_id}-ch{chapter_num:02d}-chunk{chunk_index:03d}-audio-complete"

    sqs_client.send_message(
        QueueUrl=STITCH_QUEUE_URL,
        MessageBody=json.dumps(message),
        MessageGroupId=f"{job_id}-ch{chapter_num:02d}",  # Group by job and chapter
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Triggered by SQS messages from text extraction lambda
    ...
    try:
        ...
        for record in records:
            try:
                ...
                text_s3_key = chunk_info['text_s3_key']
                text_bucket = chunk_info.get('text_bucket', TEXT_CHUNKS_BUCKET)

                logger.info(
                    f"Processing TTS for Job {job_id}, Chapter {chapter_num}, "
                ...
                # Step 3: Generate TTS with retry logic
                try:
                    pcm_data = generate_speech_with_retry(chunk_text, TTS_VOICE, TTS_MODEL)
                except Exception as e:
                    # If all retries failed, log and re-raise
                    logger.error(
                        f"Failed to generate TTS after {MAX_RETRY_ATTEMPTS} attempts "
                        f"for chunk {chunk_index+1}/{total_chunks}: {str(e)}"
                    )
                    raise
                ...
                audio_key = store_audio_chunk(
                    job_id, chapter_num, chunk_index,
                    wav_data, chapter_title, total_chunks
                )

                # Step 6: Send to stitching queue
                send_to_stitch_queue(
                    job_id, chapter_num, chapter_title,
                    chunk_index, total_chunks, audio_key
                )
